...
<body>
    ...
    <table class="info-table">
        <tr>
            ...
            <td>: {{ $realisasi->kegiatan->standar_pendidikan ?? '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            ...
            <td>: {{ $realisasi->kegiatan->kode_kegiatan ?? '-' }}</td>
        </tr>
        <tr>
            <td class="label">NAMA KEGIATAN</td>
            <td>: {{ $realisasi->kegiatan->uraian_sub_kegiatan ?? '-' }}</td>
        </tr>
        <tr>
        ...
        <tr>
            <td class="label">JUMLAH REALISASI</td>
            <td>: Rp {{ number_format($realisasi->total_realisasi, 0, ',', '.') }}</td>
        </tr>
    </table>

    <div class="judul">
        RINCIAN REALISASI PER KEGIATAN<br>
        ANGGARAN DANA BOSP TAHUN {{ $realisasi->tahun }}
    </div>
    ...
    <table class="data-table">
        ...
        <tbody>
            @foreach($realisasi->details as $index => $item)
            <tr>
        ...
        <tfoot>
            <tr>
                <th colspan="5" class="text-right">TOTAL REALISASI</th>
                <th class="text-right">Rp {{ number_format($realisasi->total_realisasi, 0, ',', '.') }}</th>
            </tr>
        </tfoot>
    </table>
    ...
    <div class="ttd-wrapper">
        <div class="ttd-box">
            ...
            <!-- Prioritas: Nama di Realisasi -> Nama di Sekolah -->
            <b><u>{{ $realisasi->nama_kepala_sekolah ?? $sekolah->nama_kepala_sekolah }}</u></b><br>
            NIP. {{ $realisasi->nip_kepala_sekolah ?? $sekolah->nip_kepala_sekolah }}
        </div>

        <div class="ttd-box right">
            {{ ucwords(strtolower($sekolah->kabupaten ?? 'Indramayu')) }}, ...................... {{ $realisasi->tahun }}<br>
            Bendahara Sekolah
            <div class="ttd-space"></div>
            <b><u>{{ $realisasi->nama_bendahara ?? $sekolah->nama_bendahara }}</u></b><br>
            NIP. {{ $realisasi->nip_bendahara ?? $sekolah->nip_bendahara }}
        </div>
